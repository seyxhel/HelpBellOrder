...
*/
  ...
  Plugin.prototype.paste = function(string) {
    ...
    // IE == 11
      ...
      // https://github.com/zammad/zammad/issues/5589
      // we check if the paste does not contain styles
      // and if yes then we remove all styles after the paste in the
      // inserted HTML which is generated by the chrome browser
      hasStyling = $('<div>').append(string).find('[style]:first').length == 1
      if (!hasStyling) {
        ...
        observer.observe(document.activeElement, { childList: true, subtree: true });
      }
