  ...
  >
    ...
    <li v-if="!items.length" class="px-4 py-2">
      <CommonLabel class="inline! truncate text-stone-200 dark:text-neutral-500">
        ...
        <template v-else-if="query && !loading">
          {{ $t('No results found') }}
        </template>
        <template v-else-if="type === 'knowledge-base' && !loading">
          {{ $t('Start typing to search in Knowledge Base…') }}
        </template>
        <template v-else-if="type === 'text' && !loading">
          {{ $t('Start typing to search for text modules…') }}
        </template>
        <template v-else-if="type === 'user' && !loading">
